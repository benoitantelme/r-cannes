...
mergeGPDPiecharts <- function(dataPerDecades, orderedDecades, size){
  # create a merge of genre per decades pie charts
  # Separate the charts in two lines and then merge them vertically
  
  nbrOfDecades <- length(dataPerDecades)
  separator <- floor(nbrOfDecades/ 2) + 1
  
  firstLine <- createHorizontalPieCharts(dataPerDecades, oderedDecades, 1, separator, size)
  secondLine <- createHorizontalPieCharts(dataPerDecades, oderedDecades, separator+1, nbrOfDecades, size)
...
createHorizontalPieCharts <- function(dataPerDecades, oderedDecades, start, stop, size){
